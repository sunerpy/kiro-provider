...
import type { KiroUsageSnapshot, ManagedAccount } from "../kiro/types.js";
 ...
 */
export function sameTokenSnapshot(expected: ManagedAccount, current: ManagedAccount): boolean {
  return (
...
export class OpenCodeAuthStore {
  ...
  markQuotaExhausted(id: string, recheckAfter: number): ManagedAccount | undefined {
    return this.withImmediateTransaction(() => {
      this.db
        .query(`
					UPDATE accounts
  ...
  scheduleQuotaRecheck(id: string, recheckAfter: number): ManagedAccount | undefined {
    return this.withImmediateTransaction(() => {
  ...
  private validateSchema(): void {
    const tables = new Set(
      this.db
        .query<TableRow, []>("SELECT name FROM sqlite_master WHERE type = 'table'")
        .all()
        .map(({ name }) => name),
    );
    ...
    const missing = [...REQUIRED_ACCOUNT_COLUMNS].filter((column) => !columns.has(column));
    if (missing.length > 0) {
      throw new OpenCodeAuthStoreError(
