...
function suggestField(unknownKey: string): string | undefined {
	...
	const prefixRelated =
		best.field.startsWith(normalized) || normalized.startsWith(best.field);
...
export function loadConfig(options: LoadConfigOptions = {}): Config {
	...
	const configPath =
		options.configPath ?? defaultConfigPath({ env, platform });
	...
	try {
		...
		sources = new Map(
			[...envOverrides.sources].filter(
				([field]) => !(field in (options.overrides ?? {})),
			),
		);
		...
		if (error instanceof z.ZodError) {
			throw new ConfigLoadError(
				`Invalid configuration: ${formatZodError(error, sources)}`,
				{ cause: error },
			);
		}
		throw error;
