...
export const ApiKeysSchema = z
  .array(z.string())
  .transform((apiKeys) =>
    apiKeys.map((apiKey) => apiKey.trim()).filter((apiKey) => apiKey.length > 0),
  )
...
export const ConfigSchema = z.object({
  ...
  runtime_endpoint_mode: z.enum(["legacy-q", "kiro-runtime"]).default("kiro-runtime"),
  ...
  account_maintenance_interval_ms: z.number().int().min(1_000).max(2_147_483_647).default(60_000),
  account_maintenance_timeout_ms: z.number().int().min(1_000).max(2_147_483_647).default(120_000),
  account_maintenance_concurrency: z.number().int().min(1).max(32).default(4),
  usage_refresh_interval_ms: z.number().int().min(1_000).max(2_147_483_647).default(900_000),
