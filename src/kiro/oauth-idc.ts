import { z } from "zod";
import { buildUrl, KIRO_AUTH_SERVICE, KIRO_CONSTANTS, normalizeRegion } from "./constants.js";
...
export async function authorizeKiroIDC(
  ...
  const endpoint = buildUrl(KIRO_AUTH_SERVICE.SSO_OIDC_ENDPOINT, effectiveRegion);
  ...
  const registration = ClientRegistrationSchema.safeParse(await registerResponse.json());
  if (!registration.success) {
    throw new KiroIDCError("Client registration response missing clientId or clientSecret", {
  ...
  const authorization = DeviceAuthorizationSchema.safeParse(await authorizationResponse.json());
  if (!authorization.success) {
    throw new KiroIDCError("Device authorization response missing required fields", {
...
export async function pollKiroIDCToken(
  ...
  for (let attempt = 1; attempt <= maxAttempts; attempt += 1) {
    ...
    if (accessToken && refreshToken) {
      return {
        refreshToken,
        accessToken,
        expiresAt: Date.now() + (tokenData.expires_in ?? tokenData.expiresIn ?? 3_600) * 1_000,
    ...
    if (!tokenResponse.ok) {
      throw new KiroIDCError(`Token request failed with status: ${tokenResponse.status}`);
    }
  ...
  throw new KiroIDCError("Token polling timed out. Authorization may have expired.");
}
