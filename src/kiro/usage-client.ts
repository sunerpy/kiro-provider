 ...
 * parking an account.
 */
function resetAtFromEpochSeconds(value: unknown, now: number): number | undefined {
 ...
 */
function usageResetAt(data: UsageLimitsResponse, now: number): number | undefined {
...
function parseUsagePayload(value: unknown, now: number): KiroUsageSnapshot {
  ...
  return {
    ...
    limitCount,
    overageCount,
    ...(parsed.data.userInfo?.email ? { email: parsed.data.userInfo.email } : {}),
