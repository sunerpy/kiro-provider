...
import { ACCOUNTS_DB_PATH, AccountsDatabase } from "../storage/accounts-db.js";
...
export async function runAccountRefresh(
  ...
  try {
    ...
    const quotaRechecker = new QuotaRechecker({
      ...
      ...(dependencies.fetchUsage ? { fetchUsage: dependencies.fetchUsage } : {}),
    ...
    const accounts = options.identifier
      ...
      : available;
    const timeoutSignal = AbortSignal.timeout(config.account_maintenance_timeout_ms);
