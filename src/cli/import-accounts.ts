...
function localCopyIsNewer(
  ...
  return local.expiresAt > incoming.expiresAt || (local.lastSync ?? 0) > (incoming.lastSync ?? 0);
}
...
function toCandidate(row: SourceAccount): AccountCandidate {
  ...
  const parsedOidcRegion =
    oidcRegion === undefined ? undefined : RegionSchema.safeParse(oidcRegion);
  ...
  return {
    ...
    account: {
      ...
      ...(optionalText(row.client_secret) ? { clientSecret: optionalText(row.client_secret) } : {}),
...
export function runImportAccounts(
  ...
  try {
    ...
    for (const rawRow of rows) {
      ...
      if (local && !options.force && localCopyIsNewer(local, candidate.account)) {
        skipped += 1;
        dependencies.stdout(
