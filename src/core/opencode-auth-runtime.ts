...
import { openCodePluginDirForDatabase, sameTokenSnapshot } from "../auth/opencode-auth-store.js";
...
import { isPermanentError, isRefreshTokenDead, toDeadReason } from "../kiro/health.js";
...
export class OpenCodeAccountManager {
  ...
  markRateLimited(account: ManagedAccount, resetTime: number): ManagedAccount | undefined {
  ...
  markQuotaExhausted(account: ManagedAccount, recheckAfter: number): ManagedAccount | undefined {
  ...
  scheduleQuotaRecheck(account: ManagedAccount, recheckAfter: number): ManagedAccount | undefined {
    const updated = this.store.scheduleQuotaRecheck(account.id, recheckAfter);
    if (updated) this.publishLatest(updated);
    else this.reconcileFromDb();
    return updated === undefined ? undefined : cloneAccount(updated);
  }
  ...
  updateQuotaUsage(
    ...
    const updated = this.store.updateQuotaUsage(account.id, usage, nextRecheckAt);
    if (updated) this.publishLatest(updated);
    else this.reconcileFromDb();
    return updated === undefined ? undefined : cloneAccount(updated);
  }
...
export class OpenCodeTokenRefresher {
  ...
  async refreshIfNeeded(
    ...
    if (!accessTokenExpired(this.accountManager.toAuthDetails(latest), this.tokenExpiryBufferMs)) {
      return latest;
    }
    return this.startOrJoinRefresh(account, false, signal);
  }

  async forceRefresh(account: ManagedAccount, signal?: AbortSignal): Promise<ManagedAccount> {
    ...
    return withOpenCodeRefreshLock(
      ...
      async () => {
        ...
        if (
          ...
          (!force && !accessTokenExpired(latestAuth, this.tokenExpiryBufferMs))
        ) {
          this.accountManager.publishLatest(latest);
          return latest;
        }
        ...
        try {
          const refreshedAuth = await refreshAccessToken(latestAuth, signal, this.proxyUrl);
          ...
          const candidate: ManagedAccount = {
            ...
            ...(refreshedAuth.email ? { email: refreshedAuth.email } : {}),
            ...(refresh.clientId ? { clientId: refresh.clientId } : {}),
            ...(refresh.clientSecret ? { clientSecret: refresh.clientSecret } : {}),
          };
          const persisted = this.store.persistRefreshedAccount(latest, candidate);
          if (!persisted) {
