...
function invokeCompromiseHandler(handler: (error: Error) => void, error: Error): void {
  try {
    handler(error);
  } catch (handlerError) {
    auditLog("error", "single_instance_compromise_handler_failed", {
      error_type: handlerError instanceof Error ? handlerError.name : typeof handlerError,
    });
  }
}
...
export function bindServiceInstanceLease<T extends StoppableServer>(
  ...
  try {
    ...
    throw new ServiceInstanceLockError("Unable to bind the service lock lifecycle to the server", {
      cause: error,
    });
  }
  return server;
