...
import { type CanonicalRequest, latestAssistantLineageFingerprint } from "../protocol/canonical.js";
import type { AnthropicMessagesRequest } from "./anthropic/request-adapter.js";
import type { ChatCompletionRequest, ResponsesRequest } from "./request-schema.js";
...
export function canonicalSessionLineage(
  ...
  return {
    ...(previousOutput !== undefined
      ? {
          lookupKeyHash: lineageHash(tenantId, protocol, request.model, previousOutput),
        }
      : {}),
    source,
...
function initialChatInput(request: ChatCompletionRequest): unknown {
  return request.messages.find((message) => message.role === "user") ?? request.messages[0];
}
...
export function responsesSessionAffinity(
  ...
  if (!tenantId) return undefined;
  for (const field of ["zuno_session_id", "kiro_provider_session_id"] as const) {
  ...
  for (const field of ["thread_id", "session_id", "conversation_id"] as const) {
  ...
  return initial === undefined
    ? undefined
    : affinityHash(tenantId, "responses", "responses.initial_input", initial);
}
...
export function chatSessionAffinity(
  ...
  if (!tenantId) return undefined;
  if (request.prompt_cache_key && request.prompt_cache_key.length > 0) {
    return affinityHash(tenantId, "chat", "chat.prompt_cache_key", request.prompt_cache_key);
  }
  if (mode !== "legacy-initial-input") return undefined;
  const initial = initialChatInput(request);
  if (initial === undefined) return undefined;
  if (request.user && request.user.length > 0) {
    return affinityHash(tenantId, "chat", "chat.user_and_initial_input", {
      user: request.user,
      initial,
    });
  }
...
export function anthropicSessionAffinity(
  ...
  if (userId) {
    return affinityHash(tenantId, "anthropic", "anthropic.user_and_initial_input", {
      userId,
      initial,
    });
  }
  return affinityHash(tenantId, "anthropic", "anthropic.initial_input", initial);
}
