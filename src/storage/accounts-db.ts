...
const MIGRATIONS: readonly Migration[] = [
  ...
  (db) => {
    if (!hasColumn(db, "accounts", "generation")) {
      db.run("ALTER TABLE accounts ADD COLUMN generation INTEGER NOT NULL DEFAULT 1");
    }
  },
...
export class AccountsDatabase {
  ...
  schemaVersion(): number {
    return this.db.query<UserVersionRow, []>("PRAGMA user_version").get()?.user_version ?? 0;
  }
  ...
  insertAccount(account: ManagedAccount): StoredAccount {
    return this.withImmediateTransaction(() => {
      const existing = this.db
        .query<GenerationRow, [string]>("SELECT generation FROM accounts WHERE id = ?")
        .get(account.id);
      const tombstone = this.db
        .query<TombstoneRow, [string]>("SELECT last_generation FROM removed_accounts WHERE id = ?")
        .get(account.id);
      const generation = Math.max(existing?.generation ?? 0, tombstone?.last_generation ?? 0) + 1;
  ...
  updateExistingAccounts(accounts: readonly StoredAccount[]): number {
    return this.withImmediateTransaction(() => {
      ...
      for (const account of accounts) {
        ...
        bindings.pop();
        changes += update.run(...bindings, id ?? account.id, account.generation).changes;
      }
      return changes;
    });
  }
  ...
  removeAccount(id: string): void {
    this.withImmediateTransaction(() => {
      ...
      const tombstone = this.db
        .query<TombstoneRow, [string]>("SELECT last_generation FROM removed_accounts WHERE id = ?")
        .get(id);
      const lastGeneration = existing?.generation ?? tombstone?.last_generation ?? 0;
      this.db.query("DELETE FROM accounts WHERE id = ?").run(id);
      this.db.query("DELETE FROM session_affinity WHERE account_id = ?").run(id);
      this.db.query("DELETE FROM output_lineage WHERE account_id = ?").run(id);
      this.db.query("DELETE FROM reasoning_replay WHERE account_id = ?").run(id);
      this.db
        .query(`
  ...
  pruneSessionAffinities(now: number = Date.now(), maxEntries = 10_000): number {
    return this.withImmediateTransaction(() =>
    ...
    this.withImmediateTransaction(() => {
      ...
      this.db
        .query(`
	      ...
	      `)
        .run(keyHash, accountId, conversationId, now, now, now + ttlMs);
      this.pruneOutputLineageInternal(now, maxEntries, keyHash, accountId, conversationId);
    });
  }

  pruneOutputLineage(now: number = Date.now(), maxEntries = 10_000): number {
    return this.withImmediateTransaction(() => this.pruneOutputLineageInternal(now, maxEntries));
  }
  ...
  getReasoningReplayRecord(tokenHash: string): ReasoningReplayRecord | undefined {
    const row = this.db
      .query<ReasoningReplayRow, [string]>("SELECT * FROM reasoning_replay WHERE token_hash = ?")
      .get(tokenHash);
  ...
  private selectSessionAffinity(keyHash: string): SessionAffinityRow | undefined {
    const row = this.db
      .query<SessionAffinityRow, [string]>("SELECT * FROM session_affinity WHERE key_hash = ?")
      .get(keyHash);
  ...
  private pruneSessionAffinitiesInternal(
    ...
    let changes = this.db
      ...
      .run(now).changes;
    const row = this.db.query<CountRow, []>("SELECT COUNT(*) AS count FROM session_affinity").get();
    ...
    if (overflow > 0) {
      const prune = preserveKey
        ...
        : this.db.query(`
	      ...
	      `);
      changes += (preserveKey ? prune.run(preserveKey, overflow) : prune.run(overflow)).changes;
    }
    return changes;
  }
  ...
  private pruneReasoningReplayInternal(
    ...
    let changes = this.db
      ...
      .run(now).changes;
    const row = this.db.query<CountRow, []>("SELECT COUNT(*) AS count FROM reasoning_replay").get();
    const overflow = Math.max(0, (row?.count ?? 0) - maxEntries);
  ...
  private pruneOutputLineageInternal(
    ...
    let changes = this.db
      ...
      .run(now).changes;
    const row = this.db.query<CountRow, []>("SELECT COUNT(*) AS count FROM output_lineage").get();
    const overflow = Math.max(0, (row?.count ?? 0) - maxEntries);
    if (overflow <= 0) return changes;
    const preserve =
    ...
    changes += (
      preserve
        ? prune.run(preserveKey, preserveAccountId, preserveConversationId, overflow)
        : prune.run(overflow)
    ).changes;
    return changes;
  }
...
export function createAccountsDatabase(path?: string): AccountsDatabase {
  return path === undefined ? new AccountsDatabase() : new AccountsDatabase(path);
}
