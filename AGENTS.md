...
- Kiro tool-result content carries text/JSON only; images are lifted to the containing user turn. Preserve tool-result order, tool IDs, status, text, and image-block order. Multiple image-bearing tool results in one user message may use this evidenced upstream projection only with `x-kiro-tool-result-image-mode: multiple-lifted` and count-only audit fields; direct user images mixed with tool-result images remain fail-closed.
...
- Fable Messages with enabled thinking and effective `display: omitted` may omit an entire conflicting signature-only upstream prefix before publishing any output. Inspect the complete prefix within 128 events and 1 MiB; preserve exact duplicate signatures normally. Omission must skip reasoning capture and token creation, emit the compatibility header and count-only audit without signature hashes, and never retry the accepted generation. Non-empty, redacted, mixed, summarized, non-Fable, oversized or late conflicts remain fatal; an already committed stream terminates with an SSE error rather than a new HTTP status.
- Admit body-bearing generation/count requests after authentication and before reading the body. The shared request-count and body-byte reservation covers parsing, account waits, generation, response consumption and bounded upstream cleanup; never release it merely because headers were returned or a cancellation was requested.
