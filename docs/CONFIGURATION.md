...
| `stream_max_attempts` | integer, `1`-`10`, default `3` | `KIRO_PROVIDER_STREAM_MAX_ATTEMPTS` | Upstream stream attempts allowed before the first semantic event (reasoning, text, or tool call) is published to the client. A stream that fails before that point is retried with the rate-limit backoff (same account first, then another account) instead of ending the response with an error; nothing is ever retried after output has been published. |
| `retry_empty_completion` | `boolean`, default `true` | `KIRO_PROVIDER_RETRY_EMPTY_COMPLETION` | When Kiro completes a stream with no reasoning, text, or tool output at all, spend one additional attempt on the same account before returning the empty completion. Counts against `stream_max_attempts`. |
