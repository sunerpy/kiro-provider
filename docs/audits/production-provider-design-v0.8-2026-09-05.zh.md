# kiro-provider v0.8 生产级协议保真设计

> 当前基线：2026-09-05。认证参考为同级目录最新
...
- 指令迁移只能显式选择 `legacy-user-prefix`；该模式证据门控弃用，不绑定
  固定删除版本。
...
- `legacy-user-prefix`：只把每个原始指令文本块用精确 `\n\n` 连接。开头和
  中间的指令前置到首个 user 回合；连续尾部指令保留在当前边界：当前是
  user/tool 时附加文本且不移动结构化输入，历史以 assistant 结果结束时才
  新建当前合成 user 回合。这样可避免调和/续跑指令被移到先前 assistant
  结果之前，也不会恢复其他旧版合并、删除、工具补偿或提示词模板。

启用 legacy 时启动日志只记录模式警告，不记录指令正文。该模式仍处于弃用
状态；只有原生指令保真能力成立或受影响客户端迁移完成后才会移除。
...
详细证据见
[Zuno 流错误接入说明](../readme/ZUNO_STREAM_ERROR_HANDOFF.zh-CN.md)、
[流错误协议契约](../STREAM_ERROR_CONTRACT.md)、
[RC.5 账号管理验收报告](kiro-provider-v0.5.0-rc.5-account-management-validation-2026-08-29.md)、
[RC.4 本地认证验收报告](kiro-provider-v0.5.0-rc.4-local-auth-maintenance-validation-2026-08-29.md)、
[RC.3 协议验收报告](kiro-provider-v0.5.0-rc.3-opus5-validation-2026-08-27.md)
和 [Kiro 原生探针](kiro-protocol-projection-probe-2026-08-26.md)。
