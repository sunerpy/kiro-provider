...
> **状态**：当前指南 · **最新仓库内验收**：Codex CLI 0.154.0（2026-09-14）
...
- Codex CLI 0.154.0 的 compaction 与 Ultra reasoning 路径；
...
Capture 只把脱敏请求形态元数据（计数、item type、role、哈希和存在性标记）
写入 owner-only 临时目录，并由退出 trap 删除；不会持久化凭据、原始请求体、
...
详见 [V3 协议兼容范围](PROTOCOL_COMPATIBILITY.zh-CN.md)、
[初始 V3 验证](../audits/kiro-provider-v3-openai-responses-validation-2026-09-05.zh.md)
与后续[回放/压缩验收](../audits/responses-replay-delivery-2026-09-14.zh.md)。
