...
| `stream_max_attempts` | 整数，`1`-`10`，默认 `3` | `KIRO_PROVIDER_STREAM_MAX_ATTEMPTS` | 首个语义事件（推理、文本或工具调用）发布给客户端之前允许的上游流尝试次数。在此之前失败的流按限流退避重试（先同账户，再换账户），不会以错误结束响应；一旦有输出已发布则绝不重试。 |
| `retry_empty_completion` | `boolean`，默认 `true` | `KIRO_PROVIDER_RETRY_EMPTY_COMPLETION` | Kiro 以完全没有推理、文本和工具输出的方式结束流时，在同账户上多尝试一次再返回空完成。计入 `stream_max_attempts`。 |
