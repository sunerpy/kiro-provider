...
`scripts/codex-smoke.sh` 把上面的流程封装成一个 fail-closed 的脚本：它用 `mktemp -d` 创建自己的 `CODEX_HOME`，在导出任何环境变量之前先校验这个目录不是 `~/.codex` 或其子目录，写入临时 `config.toml`，然后非交互运行 `codex exec`。等网关启动、账号导入完成后自行运行：
