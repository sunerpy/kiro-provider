...
`[1m]` before sending the model ID. The small-fast row maps to Sonnet 5, so it
inherits that 1M window. Explicit custom model pins are preserved. Ultra is the default; it sends `xhigh` in the validated
