  ...
  try {
      ...
      for (const [key, value] of Object.entries(record)) {
        ...
        if (key === "assistantResponseEvent") {
          ...
          if (reasoning.redactedContent)
            redactedBytes = (redactedBytes ?? 0) + reasoning.redactedContent.byteLength;
          ...
          key === "metadataEvent" ||
          key === "messageMetadataEvent" ||
          key === "meteringEvent" ||
          key === "contextUsageEvent"
        ) {
    ...
    return {
      ...
      reasoning: {
        text: reasoningText,
        ...(signature
          ? { signatureHash: hash16(signature), signatureLength: signature.length }
          : {}),
    ...
    if (value && typeof value === "object") {
      return Object.fromEntries(
        Object.entries(value as Record<string, unknown>).map(([key, item]) => [
          key,
          /email|arn|id$/i.test(key) && typeof item === "string"
            ? `<${item.length} chars>`
  ...
  const walk = (value: unknown, path: string): void => {
    if (Array.isArray(value)) {
      ...
      for (const [key, item] of Object.entries(value as Record<string, unknown>)) {
        if (/reset|renew|period|until|next|date|expire/i.test(key))
          resetLike.push({ path: `${path}.${key}`, value: item });
        walk(item, `${path}.${key}`);
      }
    }
  };
...
async function main(): Promise<void> {
  ...
  const log = (label: string, value: unknown): void => {
    ...
    console.log(`\n=== ${label} ===`);
    console.log(
      JSON.stringify(value, (_k, v) => (typeof v === "bigint" ? Number(v) : v), 2).slice(0, 6000),
    );
  };
  ...
  if (only === "all") {
    ...
    const control = await runProbe(
      ...
      conversation(
        crypto.randomUUID(),
        userInput(wire, "Reply with exactly the token KIRO_CTRL_5A21 and nothing else."),
      ),
    );
    ...
    const toolA = {
      toolSpecification: {
        name: "probe_tool_a",
        description: "Probe tool A. Echoes its marker.",
        inputSchema: {
          json: {
            type: "object",
    ...
    const toolB = {
      toolSpecification: {
        name: "probe_tool_b",
        description: "Probe tool B. Echoes its marker.",
        inputSchema: {
          json: {
            type: "object",
            properties: { marker: { type: "string" } },
    ...
    const callA = {
      toolUseId: "tooluse_probe_a_001",
      name: "probe_tool_a",
      input: { marker: "A" },
    };
    const callB = {
      toolUseId: "tooluse_probe_b_001",
      name: "probe_tool_b",
      input: { marker: "B" },
    };
    const resultA = {
      toolUseId: callA.toolUseId,
      content: [{ text: "RESULT_A_7F3C" }],
      status: "success" as const,
    };
    const resultB = {
      toolUseId: callB.toolUseId,
      content: [{ text: "RESULT_B_9E1D" }],
      status: "success" as const,
    };
    const opening = userInput(
      wire,
      "Call probe_tool_a with marker A and probe_tool_b with marker B, then repeat both tool results verbatim on one line.",
      { userInputMessageContext: { tools: [toolA, toolB] } },
    );
    const followUp = (results: (typeof resultA)[]) =>
      userInput(wire, "", {
        userInputMessageContext: { tools: [toolA, toolB], toolResults: results },
      });
    ...
    const b21Native = await runProbe(
      account,
      "B21_native_shape",
      conversation(crypto.randomUUID(), followUp([resultA, resultB]), nativeHistory),
    );
    ...
    const splitHistory: HistoryEntry[] = [
      ...
      {
        userInputMessage: userInput(wire, "", {
          userInputMessageContext: { toolResults: [resultA] },
        }),
      },
    ];
    const b21Split = await runProbe(
      account,
      "B21_split_projection",
      conversation(crypto.randomUUID(), followUp([resultB]), splitHistory),
    );
    ...
    const b21Split2 = await runProbe(
      account,
      "B21_split_calls_merged_results",
      conversation(crypto.randomUUID(), followUp([resultA, resultB]), splitHistoryMergedResults),
    );
  ...
  const sigTool = {
    toolSpecification: {
      name: "probe_tool_sig",
      description: "Probe tool. Echoes its marker.",
      inputSchema: {
  ...
  const turn1Input = userInput(wire, turn1Prompt, {
    userInputMessageContext: { tools: [sigTool] },
  });
  const turn1 = await runProbe(
    account,
    "B25_turn1_tool_call",
    conversation(convX, turn1Input),
    effortFields,
  );
  ...
  if (rawSignature && call?.toolUseId) {
    ...
    const replayed = (signature: string, includeReasoning = true): HistoryEntry[] => [
      { userInputMessage: turn1Input },
      {
        assistantResponseMessage: {
          content: turn1.text,
          ...(includeReasoning
            ? { reasoningContent: { reasoningText: { text: turn1.reasoning.text, signature } } }
            : {}),
          toolUses: [
            { toolUseId: call.toolUseId, name: call.name, input: parsedInput as ToolUse["input"] },
          ],
        },
      },
    ];
    const toolResult = {
      ...
      content: [{ text: "RESULT_S_3C9A" }],
      status: "success" as const,
    };
    const turn2 = () =>
      userInput(wire, "", {
        userInputMessageContext: { tools: [sigTool], toolResults: [toolResult] },
      });
    const same = await runProbe(
      account,
      "B25_same_conversation_valid_signature",
      conversation(convX, turn2(), replayed(rawSignature)),
      effortFields,
    );
    log("B25_same_conversation_valid_signature", { ...same, __signature: undefined });
    const other = await runProbe(
      account,
      "B25_other_conversation_valid_signature",
      conversation(crypto.randomUUID(), turn2(), replayed(rawSignature)),
      effortFields,
    );
    ...
    const bad = await runProbe(
      account,
      "B25_same_conversation_tampered_signature",
      conversation(convX, turn2(), replayed(tampered)),
      effortFields,
    );
    log("B25_same_conversation_tampered_signature", { ...bad, __signature: undefined });
    const none = await runProbe(
      account,
      "B25_same_conversation_no_reasoning",
      conversation(convX, turn2(), replayed(rawSignature, false)),
      effortFields,
    );
    ...
    const cross = await runProbe(
      secondAccount,
      "B25_other_account_other_conversation_valid_signature",
      conversation(crypto.randomUUID(), turn2(), replayed(rawSignature)),
      effortFields,
    );
    log("B25_other_account_other_conversation_valid_signature", {
      ...cross,
  ...
  writeFileSync(
    outPath,
    JSON.stringify(results, (_k, v) => (typeof v === "bigint" ? Number(v) : v), 2),
  );
