...
describe("evaluateCoverage", () => {
  test("passes when aggregate line coverage meets the minimum", () => {
    ...
    // When
    const result = evaluateCoverage(lcov, ["src/covered.ts", "src/boundary.ts"]);
  ...
  test("fails when aggregate line coverage is below the minimum", () => {
    ...
    // When
    const result = evaluateCoverage(lcov, ["src/covered.ts", "src/under-covered.ts"]);
  ...
  test("rejects malformed line counts", () => {
    ...
    // When
    const results = malformedReports.map((lcov) => evaluateCoverage(lcov, ["src/malformed.ts"]));
  ...
  test("rejects duplicate source-file records", () => {
    // Given
    const lcov = [lcovRecord("src/duplicate.ts", 1, 1), lcovRecord("src/duplicate.ts", 1, 1)].join(
      "\n",
    );
  ...
  test("rejects duplicate source-file records written with path aliases", () => {
    // Given
    const lcov = [lcovRecord("/tmp/project/src/a.ts", 1, 1), lcovRecord("src/a.ts", 1, 0)].join(
      "\n",
    );
  ...
  test("keeps files with the same basename in distinct directories separate", () => {
    // Given
    const lcov = [lcovRecord("src/a.ts", 1, 1), lcovRecord("/tmp/project/src/sub/a.ts", 1, 1)].join(
      "\n",
    );
