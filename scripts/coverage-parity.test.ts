...
import { compareCoverageExclusions, matchesCodecovPattern } from "./coverage-parity.js";
...
describe("matchesCodecovPattern", () => {
  test("supports recursive and zero-directory globstar matches", () => {
    expect(matchesCodecovPattern("src/root.test.ts", "src/**/*.test.ts")).toBe(true);
    expect(matchesCodecovPattern("src/nested/child.test.ts", "src/**/*.test.ts")).toBe(true);
    expect(matchesCodecovPattern("src/main.ts", "src/**/*.test.ts")).toBe(false);
  });
});
