...
async function runCommand(
	...
	if (exitCode === undefined) {
		...
		throw new Error(
			`Command timed out after ${timeoutMs}ms: ${command[0] ?? "unknown"}`,
		);
	}
...
async function runOpenAiResponses(
	...
	if (
		typeof first.output_text !== "string" ||
		!first.output_text.includes(firstMarker)
	) {
	...
	if (
		typeof second.output_text !== "string" ||
		!second.output_text.includes(secondMarker)
	) {
...
async function main(): Promise<void> {
	...
	try {
			...
			`${JSON.stringify(
				{
					...
					api_keys: [apiKey],
					auth_source: "local",
					opencode_auth_db_path: join(
						temporaryRoot,
						"missing-opencode-database.db",
					),
		...
		await runCommand(
			[binaryPath, "accounts", "import", "--from", sourceDbPath],
			environment,
		);
		const imported = selectImportedAccount(localDatabasePath);
		const initialState = readLocalAccountState(localDatabasePath);
		if (
			initialState.accountCount !== 1 ||
			initialState.accessToken !== staleAccessToken
		) {
		...
		if (
			!stderr.includes('"token_due_count":1') ||
			!stderr.includes('"token_refreshed_count":1')
		) {
