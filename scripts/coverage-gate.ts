...
function normalizePath(path: string): string {
  ...
  return slashPath.length === 0 ? "" : posix.normalize(slashPath).replace(/^\.\//, "");
}
...
function canonicalCoveragePath(path: string): string {
  ...
  return sourceRootIndex === -1 ? normalizedPath : normalizedPath.slice(sourceRootIndex + 1);
}
...
function parseLcov(lcovText: string): LcovParseResult {
  ...
  for (const [index, line] of lcovText.split(/\r?\n/).entries()) {
    ...
    if (line.startsWith("SF:")) {
      ...
      if (source === undefined || linesFound === undefined || linesHit === undefined) {
...
export function matchesCoveragePattern(path: string, patterns: readonly string[]): boolean {
  const normalizedPath = normalizePath(path);
...
export function evaluateCoverage(
  ...
  const recordsByFile = new Map(parsed.records.map((record) => [record.source, record] as const));
...
async function listInScopeFiles(): Promise<readonly string[]> {
  return (await Array.fromAsync(new Bun.Glob("src/**/*.ts").scan(".")))
    .map(normalizePath)
    .filter((file) => !matchesCoveragePattern(file, CANONICAL_COVERAGE_EXCLUDE))
    .sort();
}
