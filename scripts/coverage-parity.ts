import { z } from "zod";
import { CANONICAL_COVERAGE_EXCLUDE, matchesCoveragePattern } from "./coverage-gate.js";
...
export function compareCoverageExclusions(
  ...
  for (const file of [...new Set(files.map(normalizePath))].sort()) {
    ...
    const codecovIgnored = codecovPatterns.some((pattern) => matchesCodecovPattern(file, pattern));
...
async function runCoverageParity(): Promise<number> {
  ...
  if (result.ok) {
    console.log("coverage-parity: PASS (canonical and Codecov effective ignore sets match)");
    return 0;
  }
