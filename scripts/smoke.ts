...
function responseOutputItems(response: Readonly<Record<string, unknown>>): readonly Readonly<Record<string, unknown>>[] {
...
function parseCompletedResponse(body: unknown): Readonly<Record<string, unknown>> {
  ...
  if (response.status !== 'completed') {
    throw new SmokeError(`response status must be "completed", received ${JSON.stringify(response.status)}`)
  }
  return response
}

async function checkResponses(options: Options): Promise<CheckOutcome> {
  const response = parseCompletedResponse(await requestJson(`${options.baseUrl}/v1/responses`, {
    method: 'POST', headers: headers(options.key), body: responsesBody(options.model, false)
  }))
...
async function checkResponsesStreaming(options: Options): Promise<CheckOutcome> {
  const response = await fetch(`${options.baseUrl}/v1/responses`, {
    method: 'POST', headers: headers(options.key), body: responsesBody(options.model, true)
  })
  await requireSuccess(response)
...
async function checkMessages(options: Options): Promise<CheckOutcome> {
  const body = requireRecord(await requestJson(`${options.baseUrl}/v1/messages`, {
    method: 'POST', headers: headers(options.key), body: messagesBody(options.messagesModel)
  }), 'messages response')
...
async function legacyChatResponse(options: Options, body: string): Promise<Response | 'disabled'> {
  const response = await fetch(`${options.baseUrl}/v1/chat/completions`, {
    method: 'POST', headers: headers(options.key), body
  })
...
function consumeChatFrame(frame: SseFrame, state: { content: string; chunks: number; done: number }): void {
  if (frame.data === '[DONE]') {
    state.done += 1
    return
  }
...
async function checkReasoning(options: Options): Promise<CheckOutcome> {
  const response = await legacyChatResponse(options, chatBody(options.reasoningModel, false, 'high'))
  if (response === 'disabled') return skipLegacy()
  await requireSuccess(response)
