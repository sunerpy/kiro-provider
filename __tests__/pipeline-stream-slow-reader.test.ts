...
function isRequestIdleTimeoutLease(
	value: unknown,
): value is RequestIdleTimeoutLease {
	return (
		isRecord(value) &&
...
function requireSection13Infrastructure(): Section13Infrastructure {
	...
	const createRequestIdleTimeoutLease = Reflect.get(
		exports,
	...
	return {
		buildServeOptions(config, dependencies) {
			const options: unknown = Reflect.apply(buildServeOptions, undefined, [
				config,
				dependencies,
			]);
			if (
				!isRecord(options) ||
				typeof Reflect.get(options, "fetch") !== "function"
			) {
				throw new TypeError(
					"buildServeOptions must return Bun serve options with a fetch handler",
				);
			}
			return options as unknown as Bun.Serve.Options<undefined>;
		},
		createRequestIdleTimeoutLease(request, server) {
			const lease: unknown = Reflect.apply(
				createRequestIdleTimeoutLease,
				undefined,
				[request, server],
			);
			if (!isRequestIdleTimeoutLease(lease)) {
				throw new TypeError(
					"createRequestIdleTimeoutLease must return a RequestIdleTimeoutLease",
				);
			}
			return lease;
		},
...
async function cleanupResources(
	...
	const results = await Promise.allSettled([
		...
		...observedPromises.map((observed) =>
			observed.settlement.then(() => undefined),
		),
	...
	return withTimeout(
		new Promise<PausedResponse>((resolve, reject) => {
			...
			const onClose = (): void => {
				cleanup();
				reject(
					new Error("slow-reader socket closed before headers and one chunk"),
				);
			};
...
function makeDependencies(
	...
	return {
		...
		dependencies: {
			...
			makeClient,
			createRequestIdleTimeoutLease:
				kind === "baseline" ? baselineMaker : treatmentMaker,
		},
	};
}
...
async function runArm(
	...
	const assembled = makeDependencies(
		kind,
		infrastructure,
		sdk,
		secondAcquired,
	...
	const serveOptions = infrastructure.buildServeOptions(
		testConfig(),
		assembled.dependencies,
	);
	if (Object.hasOwn(serveOptions, "idleTimeout")) {
		throw new TypeError(
			`${kind} buildServeOptions must omit Bun's global idleTimeout`,
		);
	}
	...
	try {
		...
		const pausedResponsePromise = observePromise(
			readHeadersAndFirstChunk(slowSocket.socket),
		);
		...
		await withTimeout(
			sdk.state.returned,
			CLEANUP_WINDOW_MS,
			`${kind} SDK iterator return cleanup`,
		);
		if (kind === "treatment") {
			await withTimeout(
...
function assertCommonApplicationBehavior(result: ArmResult): void {
	...
	expect(result.headers.toLowerCase()).toContain(
		"content-type: text/event-stream",
	);
	expect(result.firstChunk).toContain("data: ");
	expect(result.iteratorReturnCalls).toBe(1);
	expect(result.queueAcquireElapsedMs).toBeLessThan(
		QUEUE_ACQUIRE_WINDOW_MS,
	);
}
...
describe("raw TCP streaming slow-reader application-resource release", () => {
	test(
		"treatment preserves baseline socket behavior while releasing the queue, cleaning the iterator, and restoring its lease",
		async () => {
			...
			if (
				baseline.socketTerminatedDuringWindow &&
				!treatment.socketTerminatedDuringWindow
			) {
				throw new Error(
