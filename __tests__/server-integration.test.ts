...
} from "../src/server/routes/chat-completions.js";
...
describe("POST /v1/chat/completions", () => {
  test("reframes successful NDJSON as SSE with one terminal sentinel", async () => {
    ...
    expect(jsonFrames.some((chunk) => chunk.choices?.[0]?.finish_reason === "tool_calls")).toBe(
      true,
    );
  ...
  test("restores route resources when Chat adapter construction fails synchronously", async () => {
    // Given
    const order: string[] = [];
    let makerCalls = 0;
    let disableCalls = 0;
    ...
    let cleanupHeldReader: (() => Promise<void>) | undefined;
    let pipelineOptions: RunChatCompletionOptions | undefined;
    const dependencies: ChatCompletionDependencies = {
      accountManager: new FakeAccountManager(),
      tokenRefresher: new FakeTokenRefresher(),
      makeClient: () => ({
        async send(): Promise<SdkStreamResponse> {
          throw new TypeError("Future runPipeline seam was ignored");
        },
      }),
      async runPipeline(options): Promise<Response> {
        runPipelineCalls += 1;
        ...
        const heldReader = pipelineResponse.body.getReader();
        cleanupHeldReader = async (): Promise<void> => {
          try {
    ...
    try {
      // When
      const result = handleChatCompletions(
      ...
      expect({
        ...
        runPipelineCalls,
        order,
        pipelineOptionsCaptured: pipelineOptions !== undefined,
      }).toEqual({
        makerCalls: 1,
        disableCalls: 1,
        restoreCalls: 1,
...
describe("HTTP boundary errors", () => {
  ...
  test.each([
    ...
    ["missing tool_call_id", validBody({ messages: [{ role: "tool", content: "result" }] })],
    [
    ...
    ["assistant without content or tool calls", validBody({ messages: [{ role: "assistant" }] })],
  ...
  test("returns 504 when a streamed request body stalls beyond the ingress deadline", async () => {
    ...
    // When
    const response = await fetch(`${timeoutServer.baseUrl}/v1/chat/completions`, {
      method: "POST",
      headers: {
        Authorization: `Bearer ${API_KEY}`,
        "Content-Type": "application/json",
