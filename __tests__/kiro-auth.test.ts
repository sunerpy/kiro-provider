...
describe("accessTokenExpired", () => {
  test("returns true when the access token is missing", () => {
    expect(accessTokenExpired({ ...baseAuth(), access: "", expires: Date.now() + 999_999 })).toBe(
      true,
    );
  });
  ...
  test("returns true after expiry", () => {
    expect(accessTokenExpired({ ...baseAuth(), expires: Date.now() - 1_000 }, 120_000)).toBe(true);
  });

  test("returns true inside the expiry buffer", () => {
    expect(accessTokenExpired({ ...baseAuth(), expires: Date.now() + 60_000 }, 120_000)).toBe(true);
  });
