...
describe("classifyError gateway statuses", () => {
	test.each([502, 503, 504])(
		...
		(status) => {
			expect(
				classifyError(error({ status }), context({ serverErrorCount: 1 })),
			).toEqual({ action: "retry", status, retryAfterMs: 1_000 });
			expect(
				classifyError(error({ status }), context({ serverErrorCount: 4 })),
			).toEqual({ action: "retry", status, retryAfterMs: 8_000 });
			expect(
				classifyError(error({ status }), context({ serverErrorCount: 5 })),
			).toEqual({ action: "switch", status });
		},
	);
...
describe("classifyError transport codes", () => {
	...
	test("recognizes 'timed out' and other socket messages when no code exists", () => {
		...
		expect(isNetworkError(error({ message: "getaddrinfo EAI_AGAIN q.us-east-1.amazonaws.com" }))).toBe(true);
	...
	test("fails a transport error once the retry cap is reached", () => {
		expect(
			classifyError(error({ code: "ECONNREFUSED" }), context({ retryCount: 3 })),
