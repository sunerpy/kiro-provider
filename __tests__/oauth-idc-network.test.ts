...
import { authorizeKiroIDC, pollKiroIDCToken } from "../src/kiro/oauth-idc.js";
...
function captureFetch(responder: (request: CapturedRequest, index: number) => Response): {
...
describe("authorizeKiroIDC with mocked network", () => {
  ...
  test("registers a client and requests device authorization", async () => {
    ...
    expect(calls[1]?.url).toBe("https://oidc.eu-west-1.amazonaws.com/device_authorization");
  ...
  test("passes the proxy to client registration and device authorization", async () => {
    ...
    await authorizeKiroIDC("us-east-1", undefined, "http://p:1080");
  ...
  test("rejects failed and malformed authorization responses", async () => {
    globalThis.fetch = captureFetch(() => new Response("nope", { status: 400 })).fn;
    await expect(authorizeKiroIDC("us-east-1")).rejects.toThrow("Client registration failed: 400");

    globalThis.fetch = captureFetch(() => json({ clientId: "only-id" })).fn;
    await expect(authorizeKiroIDC("us-east-1")).rejects.toThrow("missing clientId or clientSecret");

    globalThis.fetch = captureFetch((_request, index) =>
      index === 0 ? json({ clientId: "c", clientSecret: "s" }) : json({ userCode: "code" }),
    ).fn;
    await expect(authorizeKiroIDC("us-east-1")).rejects.toThrow("missing required fields");
  });
});
...
describe("pollKiroIDCToken with mocked network", () => {
  ...
  test("passes the proxy to token polling", async () => {
    const { fn, calls } = captureFetch(() => json({ access_token: "a", refresh_token: "r" }));
    globalThis.fetch = fn;
  ...
  test("handles slow down and terminal OAuth errors", async () => {
    globalThis.fetch = captureFetch((_request, index) =>
      index === 0 ? json({ error: "slow_down" }) : json({ accessToken: "a", refreshToken: "r" }),
    ).fn;
    await expect(
  ...
  test("times out after the device code polling budget", async () => {
    ...
    await expect(
      pollKiroIDCToken("client", "secret", "device", 5, 10, "us-east-1", immediateSleep),
