...
describe("ConfigSchema", () => {
  test("applies defaults when only api_keys is provided", () => {
    ...
    expect(config).toEqual({
      ...
      session_affinity_mode: "explicit-only",
      kiro_prompt_cache_mode: "server-auto",
      proxy_url: null,
      sdk_http_keep_alive: false,
      enforce_single_instance: true,
      instance_lock_path: null,
  ...
  test.each([
    ...
    "HTTP://127.0.0.1:1080",
    "HTTPS://p:8443",
  ])("accepts the HTTP(S) proxy URL %s", (proxyUrl) => {
  ...
  test.each(["abc", "ftp://x"])("rejects the invalid proxy URL %s", (proxyUrl) => {
    expect(() => ConfigSchema.parse({ api_keys: ["sk-test"], proxy_url: proxyUrl })).toThrow(
      /proxy_url|http\(s\)/i,
    );
  });
  ...
  test("bounds quota and account maintenance concurrency", () => {
    for (const field of ["quota_recheck_concurrency", "account_maintenance_concurrency"] as const) {
  ...
  test("maps every string, number, list, proxy, and boolean environment field", () => {
    const config = loadConfig({
      ...
      env: {
        ...
        KIRO_PROVIDER_PROMPT_CACHE_MODE: "explicit-checkpoints",
        ...
        KIRO_PROVIDER_ACCOUNT_INFERENCE_CONCURRENCY: "3",
        ...
        KIRO_PROVIDER_STOP_ON_OVERAGE: "false",
        KIRO_PROVIDER_OVERAGE_THRESHOLD: "5",
        ...
        KIRO_PROVIDER_STREAM_MAX_ATTEMPTS: "4",
        KIRO_PROVIDER_RETRY_EMPTY_COMPLETION: "false",
        KIRO_PROVIDER_MAX_REQUEST_BODY_BYTES: "2097152",
        KIRO_PROVIDER_MAX_INFLIGHT_REQUESTS: "24",
        KIRO_PROVIDER_MAX_INFLIGHT_REQUEST_BODY_BYTES: "67108864",
    ...
    expect(config).toEqual({
      ...
      kiro_prompt_cache_mode: "explicit-checkpoints",
      ...
      account_inference_concurrency: 3,
      ...
      max_inflight_request_body_bytes: 67108864,
