...
function corruptColumn(path: string, id: string, column: 'region' | 'oidc_region', value: string): void {
...
describe('validatedRegions', () => {
  test('accepts known regions and rejects hostname-shaped values with an audit warning', () => {
    ...
    try {
      ...
      expect(validatedRegions({ id: 'a', region: 'us-east-1', oidc_region: 'ap-southeast-1' })).toEqual({
        region: 'us-east-1',
        oidcRegion: 'ap-southeast-1'
      })
      expect(validatedRegions({ id: 'a', region: HOSTILE_REGION, oidc_region: null })).toBeUndefined()
      expect(validatedRegions({ id: 'a', region: 'us-east-1', oidc_region: HOSTILE_REGION })).toBeUndefined()
      expect(validatedRegions({ id: 'a', region: '', oidc_region: null })).toBeUndefined()

      const events = warn.mock.calls.map((call) => JSON.parse(String(call[0])) as Record<string, unknown>)
