...
describe('AccountsDatabase account removal cascade', () => {
  test('removes output lineage and reasoning replay rows owned by the account only', () => {
    ...
    database.insertReasoningReplay(replayRecord({ tokenHash: 'token-a', accountId: 'account-a' }), 100, 1_000)
    ...
    expect(database.findReasoningReplayByChatHash('tenant-1', 'claude-opus-5', 'chat-hash-1', 2_000)).toEqual([])
    expect(database.activeReasoningReplayKeyIds(2_000)).toEqual(['rk_other'])
    expect(database.resolveOutputLineage('lineage-b', 2_000)).toMatchObject({ accountId: 'account-b' })
