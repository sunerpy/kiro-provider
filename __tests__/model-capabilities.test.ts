...
function model(modelId: string, overrides: Partial<KiroAvailableModel> = {}): KiroAvailableModel {
  return {
    modelId,
    modelName: modelId,
...
function service(
  listModels: (auth: KiroAuthDetails, region: string) => Promise<KiroAvailableModelsResponse>,
...
describe("ModelCapabilityService", () => {
  ...
  test("keeps account-specific availability for routing", async () => {
    const capabilities = service(async (details) => ({
      models:
        details.access === "access-b" ? [model("future-model-2")] : [model("claude-sonnet-4.5")],
    }));
    const first = account("a");
    const second = account("b");

    expect(
      await capabilities.ensureAccountModel(first, auth(first), "future-model-2"),
    ).toMatchObject({ supported: false, source: "live" });
    expect(
      await capabilities.ensureAccountModel(second, auth(second), "future-model-2"),
    ).toMatchObject({ supported: true, source: "live" });
    expect([...(capabilities.eligibleAccountIds("future-model-2", ["a", "b"]) ?? [])]).toEqual([
      "b",
    ]);
  });
  ...
  test("uses stale live data after a refresh failure", async () => {
    ...
    const selected = account("a");
    await capabilities.ensureAccountModel(selected, auth(selected), "future-model-3");
    await Bun.sleep(2);
  ...
  test("falls back only to the static compatibility set when no live catalog exists", async () => {
    ...
    expect(
      await capabilities.ensureAccountModel(selected, auth(selected), "claude-opus-5"),
    ).toEqual({
      supported: true,
      source: "static",
    ...
    expect(
      await capabilities.ensureAccountModel(selected, auth(selected), "unpublished-guess"),
