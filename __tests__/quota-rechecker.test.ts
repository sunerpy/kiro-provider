...
class FakeRefresher {
  ...
  async forceRefresh(selected: ManagedAccount, _signal?: AbortSignal): Promise<ManagedAccount> {
    this.forceCalls += 1;
...
describe("QuotaRechecker", () => {
  test("restores a due account only after the authoritative usage snapshot recovers", async () => {
    ...
    await service.recheckDueAccounts([exhausted], new AbortController().signal);
  ...
  test("force-refreshes once after an invalid bearer usage response", async () => {
    ...
    const service = rechecker(manager, refresher, async (auth) => {
      ...
      if (accessTokens.length === 1) {
        throw new KiroUsageError("The bearer token included in the request is invalid", 403);
      }
      return { usedCount: 0, limitCount: 100, overageCount: 0 };
    });
    ...
    expect(accessTokens).toEqual(["account-a-access", "account-a-rotated-access"]);
    expect(isQuotaExhausted(exhausted)).toBe(false);
  });
  ...
  test("periodically refreshes a stale non-exhausted usage snapshot", async () => {
    ...
    await service.syncDueAccounts([healthy], new AbortController().signal);
  ...
  test("manual refresh bypasses fresh and future-quota schedules", async () => {
    ...
    const summary = await service.refreshAccounts([fresh, exhausted], new AbortController().signal);
  ...
  test("manual refresh reports token renewal without exposing credentials", async () => {
    ...
    const summary = await service.refreshAccounts([selected], new AbortController().signal);
  ...
  test("manual refresh emits terminal timeout results for unstarted accounts", async () => {
    ...
    const summary = await service.refreshAccounts([selected], controller.signal);
