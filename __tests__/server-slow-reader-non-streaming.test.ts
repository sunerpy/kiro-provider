...
async function cleanupTrackedSockets(
	activeSockets: Set<TrackedSocket>,
): Promise<void> {
	const sockets = [...activeSockets];
	const results = await Promise.allSettled(
		sockets.map((tracked) => destroySocketAndWait(tracked)),
	);
...
function isCreateRequestIdleTimeoutLease(
	value: unknown,
): value is CreateRequestIdleTimeoutLease {
	return typeof value === "function";
}

function requireServerInfrastructure(): ServerInfrastructure {
	const buildServeOptions: unknown = Reflect.get(
		serverApp,
		"buildServeOptions",
	);
	...
	return new Promise((resolve, reject) => {
		...
		const timer = setTimeout(
			() =>
				fail(
					new Error(
						`Response headers were not received within ${HEADER_BOUND_MS}ms`,
					),
				),
			HEADER_BOUND_MS,
		);
...
test(
	"non-streaming slow readers preserve header progress and treatment restores its request lease",
	async () => {
		...
		try {
			...
			if (cleanupFailures.length > 0) {
				throw new AggregateError(
					cleanupFailures,
					"Non-streaming raw TCP test cleanup failed",
				);
			}
		},
	15_000,
