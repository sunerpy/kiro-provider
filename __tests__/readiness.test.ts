...
function accountManager(count: number, throwOnReconcile = false): PipelineAccountManager {
  return {
...
describe("readiness route", () => {
  test("reports every fail-closed dependency state", async () => {
    ...
    const notWritable = handleReadiness(accountManager(1), replayStore(false, true));
    ...
    const noKeyring = handleReadiness(accountManager(1), replayStore(true, false));
    ...
    const missingKey = handleReadiness(accountManager(1), replayStore(true, true, ["rk_missing"]));
  ...
  test("returns ready only when accounts, database, and keyring are usable", async () => {
    ...
    const withReplay = handleReadiness(accountManager(1), replayStore(true, true));
