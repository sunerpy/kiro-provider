...
describe('TokenRefresher', () => {
  ...
  test('deduplicates concurrent refreshes across different generations of the same account', async () => {
    ...
    // Then
    expect(fetchCalls).toBe(1)
    expect(results.map(({ accessToken }) => accessToken)).toEqual(['shared-access', 'shared-access'])
  ...
  test('skips the network when a stale snapshot is force-refreshed after another writer rotated', async () => {
    ...
    // Then
    expect(fetchCalls).toBe(0)
    expect(result).toMatchObject({ refreshToken: 'rotated-refresh', accessToken: 'rotated-access' })
  })
  ...
  test('a joiner still receives the refreshed account after the first caller aborts', async () => {
    ...
    await started.promise
    const joiner = refresher.refreshIfNeeded(fixture.account, fixture.auth, new AbortController().signal)
  ...
  test('a successful refresh heals health and persists the discovered email', async () => {
    ...
    // When
    const result = await refresher.refreshIfNeeded(fixture.account, fixture.manager.toAuthDetails(fixture.account))
