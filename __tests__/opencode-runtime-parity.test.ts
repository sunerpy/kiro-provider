...
): { readonly manager: OpenCodeAccountManager; readonly store: OpenCodeAuthStore; readonly path: string } {
...
function destroySpy(auth: ManagedAccount): ReturnType<typeof spyOn> {
	const client = createSdkClient(toAuthDetails(auth), "us-east-1", undefined, undefined, undefined, auth.id);
	const handler = client.config.requestHandler;
	if (!(handler instanceof NodeHttpHandler)) throw new TypeError("expected a NodeHttpHandler transport");
...
describe("OpenCodeAccountManager parity with AccountManager", () => {
	test("countSelectableAccounts agrees with the local manager for every eligibility set", () => {
		...
		for (const eligible of eligibilitySets) {
			expect(manager.countSelectableAccounts(eligible)).toBe(local.countSelectableAccounts(eligible));
		}
	...
	test("selection sequences match the local manager for every strategy", () => {
		...
		for (const strategy of ["sticky", "round-robin", "lowest-usage"] as const) {
			...
			const steps: readonly (string | undefined)[] = [undefined, "B", undefined, undefined, "missing", undefined];
	...
	test("a refresh-token-dead reason writes the same binary marker in both stores", () => {
		...
		const marker = { isHealthy: false, failCount: 10, unhealthyReason: reason, recoveryTime: undefined };
