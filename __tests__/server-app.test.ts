...
function config(proxyUrl: string | null, overrides: Partial<Config> = {}): Config {
...
describe("buildServerDeps", () => {
  ...
  test("constructs the production token refresher when no refresher factory is supplied", () => {
    ...
    try {
      ...
      expect(dependencies.accountMaintenance?.constructor.name).toBe("AccountMaintenanceService");
    } finally {
      database.close();
    }
  });
  ...
  test("passes quota probe tuning and the resolved proxy to its production assembly", () => {
    ...
    try {
      ...
      const dependencies = buildServerDeps(
        ...
        {
          createDatabase: () => database,
          createQuotaRechecker: (_accountManager, _tokenRefresher, resolvedConfig, proxyUrl) => {
            captured = {
              intervalMs: resolvedConfig.quota_recheck_interval_ms,
              usageRefreshIntervalMs: resolvedConfig.usage_refresh_interval_ms,
          ...
          createAccountMaintenance: (
            ...
            maintenanceConfig = {
              ...
              intervalMs: resolvedConfig.account_maintenance_interval_ms,
              timeoutMs: resolvedConfig.account_maintenance_timeout_ms,
              concurrency: resolvedConfig.account_maintenance_concurrency,
            };
...
class ThrowingAccountManager implements PipelineAccountManager {
  ...
  markUnhealthy(_account: ManagedAccount, _reason: string, _recoveryTime?: number): void {}
}
...
describe("buildServeOptions", () => {
  test("omits Bun's global idleTimeout while preserving the server wiring", () => {
    ...
    const options: Bun.Serve.Options<undefined> = buildServeOptions(parsed, dependencies);
...
describe("createApp", () => {
  ...
  const legacyChatApp = createApp(config(null, { enable_legacy_chat_completions: true }), {
    accountManager: new ThrowingAccountManager(),
    tokenRefresher: passThroughRefresher,
  });
