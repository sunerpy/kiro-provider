...
import { ACCOUNTS_DB_PATH, type StoredAccount } from "../src/storage/accounts-db.js";
...
function refreshSummary(accounts: readonly StoredAccount[]): AccountRefreshSummary {
  return {
    startedAt: 1,
    completedAt: 2,
  ...
  const deps: CliDependencies = {
    ...
    runAccountRefresh: async (_loadedConfig, options) => {
      ...
      const selected = options.identifier
        ? accounts.filter(
            (candidate) =>
              candidate.id === options.identifier || candidate.email === options.identifier,
          )
        : accounts;
...
describe("parseCliArgs", () => {
  ...
  test("parses accounts import options", () => {
    expect(parseCliArgs(["accounts", "import", "--from", "/tmp/opencode.db", "--force"])).toEqual({
  ...
  test("parses account listing, refresh, relogin, and confirmed removal", () => {
    ...
    expect(parseCliArgs(["accounts", "refresh", "dev@example.com"])).toEqual({
      kind: "accounts-refresh",
...
describe("main", () => {
  ...
  test("prints the actual address only after the server binds", async () => {
    ...
    expect(exitCode).toBe(0);
    expect(callOrder).toEqual(["startServer", "stdout:Listening on http://127.0.0.1:41237"]);
  });
  ...
  test("does not print Listening when the server bind fails", async () => {
    ...
    expect(harness.stderr).toEqual(["Failed to start server. Is port 8787 in use?"]);
  });
  ...
  test("keeps serve fail-closed when api_keys validation fails", async () => {
    ...
    expect(exitCode).toBe(1);
    expect(harness.served).toHaveLength(0);
    expect(harness.stderr).toEqual(["Invalid configuration: api_keys: Required"]);
  });
  ...
  test("lists account IDs and generations in details or JSON mode", async () => {
    ...
    expect(await main(["accounts", "list", "--details"], detailed.deps)).toBe(0);
  ...
  test("refreshes all accounts or one resolved account", async () => {
    ...
    expect(await main(["accounts", "refresh", "--all"], all.deps)).toBe(0);
    expect(all.refreshes).toEqual([{}]);
    expect(all.stdout[0]).toContain("Refreshed 1 accounts");

    expect(await main(["accounts", "refresh", "dev@example.com", "--json"], one.deps)).toBe(0);
  ...
  test("returns a failure when a manual account refresh is incomplete", async () => {
    ...
    const exitCode = await main(["accounts", "refresh", "account-1"], dependencies);
  ...
  test("dispatches accounts import with an isolated database", async () => {
    ...
    const exitCode = await main(["accounts", "import", "--from", "/tmp/opencode.db"], harness.deps);

    expect(exitCode).toBe(0);
    expect(harness.loaded).toHaveLength(0);
    expect(harness.imports).toEqual([{ from: "/tmp/opencode.db", force: false }]);
    expect(harness.stderr).toEqual([
  ...
  test("imports accounts when api_keys validation would fail", async () => {
    ...
    const exitCode = await main(["accounts", "import", "--from", "/tmp/opencode.db"], dependencies);
  ...
  test("removes accounts when api_keys validation would fail", async () => {
    ...
    const exitCode = await main(["accounts", "remove", "dev@example.com"], dependencies);
  ...
  test("opens one environment-derived database path for every accounts command", async () => {
    ...
    await main(["accounts", "import", "--from", "/tmp/opencode.db"], harness.deps);
    await main(["accounts", "remove", "dev@example.com"], harness.deps);

    expect(harness.dbPaths).toEqual([ACCOUNTS_DB_PATH, ACCOUNTS_DB_PATH, ACCOUNTS_DB_PATH]);
  });

  test("removes an account by email through the tombstone API", async () => {
    const harness = createHarness([account()]);

    const exitCode = await main(["accounts", "remove", "dev@example.com"], harness.deps);

    expect(exitCode).toBe(0);
    expect(harness.removed).toEqual(["account-1"]);
    expect(harness.confirmations[0]).toContain("account-1");
    expect(harness.stdout).toEqual(["Removed account dev@example.com [account-1]"]);
  });
  ...
  test("--yes removes without prompting", async () => {
    ...
    const exitCode = await main(["accounts", "remove", "account-1", "--yes"], harness.deps);
  ...
  test("rejects ambiguous email identifiers and prints matching IDs", async () => {
    const harness = createHarness([account({ id: "account-a" }), account({ id: "account-b" })]);

    const exitCode = await main(["accounts", "remove", "DEV@example.com", "--yes"], harness.deps);
  ...
  test("returns a failure without removing anything when an account is not found", async () => {
    ...
    const exitCode = await main(["accounts", "remove", "missing@example.com"], harness.deps);
...
describe("runLogin", () => {
  test.each([
    ...
    "passes the $label proxy URL to device authorization and polling",
  ...
  test("relogin verifies identity, preserves the account ID, and removes exact duplicates", async () => {
    ...
    const result = await runLogin(
      ...
      { replaceAccount: selected },
      {
        authorize: async (region, startUrl) => ({
          verificationUrl: "https://device.example/verify",
          verificationUriComplete: "https://device.example/verify?code=RELOGIN",
          userCode: "RELOGIN",
          deviceCode: "device-code",
