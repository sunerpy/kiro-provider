...
describe("ConfigSchema numeric bounds", () => {
	test("covers every integer environment variable", () => {
		const integerEnvFields = CONFIG_ENV_VARIABLES.filter(
			({ kind }) => kind === "integer",
		).map(({ field }) => field as string);
		expect([...integerEnvFields].sort()).toEqual(
			numericFields.map(({ field }) => field).sort(),
		);
	});

	test.each(numericFields)(
		"accepts the closed range for $field",
		({ field, min, max }) => {
	...
	test("rejects zero for every field whose minimum is at least one", () => {
		for (const { field, min } of numericFields) {
			if (min < 1) continue;
			expect(
				ConfigSchema.safeParse({ api_keys: ["sk-test"], [field]: 0 }).success,
			).toBe(false);
		}
	});
	...
	test("accepts zero retries but rejects negative retries", () => {
		expect(
			ConfigSchema.safeParse({ api_keys: ["sk-test"], rate_limit_max_retries: 0 })
				.success,
		).toBe(true);
		expect(
			ConfigSchema.safeParse({ api_keys: ["sk-test"], rate_limit_max_retries: -1 })
				.success,
		).toBe(false);
	});
	...
	test("rejects the port values the audit found accepted and keeps 0 as ephemeral", () => {
		for (const port of [70_000, 8787.5, -1, 65_536]) {
			expect(
				ConfigSchema.safeParse({ api_keys: ["sk-test"], port }).success,
			).toBe(false);
		}
...
describe("ConfigSchema enums and strings", () => {
	test("accepts every supported region and rejects unknown regions", () => {
		expect(
			ConfigSchema.parse({ api_keys: ["sk-test"], default_region: "eu-west-1" })
				.default_region,
		).toBe("eu-west-1");
		for (const region of ["moon-1", "US-EAST-1", "", " us-east-1"]) {
			expect(
				ConfigSchema.safeParse({ api_keys: ["sk-test"], default_region: region })
					.success,
			).toBe(false);
		}
	});
	...
	test("narrows log_level to debug, info, warn, error", () => {
		for (const level of ["debug", "info", "warn", "error"] as const) {
			expect(
				ConfigSchema.parse({ api_keys: ["sk-test"], log_level: level }).log_level,
			).toBe(level);
		}
		for (const level of ["verbose", "INFO", "trace", ""]) {
			expect(
				ConfigSchema.safeParse({ api_keys: ["sk-test"], log_level: level })
					.success,
			).toBe(false);
		}
	});
	...
	test("rejects an empty or whitespace host", () => {
		for (const host of ["", "   "]) {
			expect(
				ConfigSchema.safeParse({ api_keys: ["sk-test"], host }).success,
			).toBe(false);
		}
		expect(
			ConfigSchema.parse({ api_keys: ["sk-test"], host: " 0.0.0.0 " }).host,
		).toBe("0.0.0.0");
	});
});
...
describe("loadConfig unknown keys", () => {
	test("rejects an unknown key and suggests the closest field", () => {
		...
		expect((caught as Error).message).toContain(
			'did you mean "enable_legacy_chat_completions"?',
		);
	});
	...
	test("lists every unknown key and omits a suggestion for unrelated names", () => {
		...
		expect(message).not.toContain(
			'"completely_unrelated_setting_name" (did you mean',
		);
	});
...
describe("platform config paths", () => {
	test("uses XDG_CONFIG_HOME or ~/.config on POSIX", () => {
		...
		expect(
			platformConfigRoot({ platform: "darwin", env: {}, homeDirectory: "/Users/u" }),
		).toBe("/Users/u/.config");
		expect(
			platformConfigRoot({
				platform: "linux",
	...
	test("uses APPDATA or the roaming profile on win32", () => {
		...
		expect(
			platformConfigRoot({ platform: "win32", env: {}, homeDirectory: "/home/u" }),
		).toBe(join("/home/u", "AppData", "Roaming"));
		expect(legacyConfigRoot({ env: {}, homeDirectory: "/home/u" })).toBe(
			"/home/u/.config",
		);
	});
	...
	test("prefers the APPDATA config on win32 but falls back to the legacy ~/.config file", () => {
		...
		expect(
			defaultConfigPath({ ...options, exists: (path) => path === legacy }),
		).toBe(legacy);
