...
): { readonly deps: CliDependencies; readonly stdout: string[]; readonly stderr: string[] } {
	const stdout: string[] = [];
	const stderr: string[] = [];
...
describe("accounts import options", () => {
	test("rejects the removed --config option", () => {
		expect(() =>
			parseCliArgs(["accounts", "import", "--config", "/tmp/config.json"]),
		).toThrow("Unknown option '--config'");
	});
...
describe("serve --port", () => {
	test.each(["0x1f90", "1e3", "8787abc", " "])(
		"rejects the non-decimal port %s",
		(port) => {
			expect(() => parseCliArgs(["serve", "--port", port])).toThrow(
				`Invalid port: ${port}`,
			);
		},
	);
