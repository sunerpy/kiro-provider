...
const refresher: PipelineTokenRefresher = {
  refreshIfNeeded(selected: ManagedAccount, _auth: KiroAuthDetails): Promise<ManagedAccount> {
  ...
  test("reuses the persisted account and Kiro conversation id for one session", async () => {
    ...
    const makeClient = (
      ...
      send(command) {
        ...
        return Promise.resolve(responseFrom([{ assistantResponseEvent: { content: "ok" } }]));
      },
    });
    const options = {
      body: BODY,
