...
async function captureAudit(run: () => Promise<void>): Promise<Array<Record<string, unknown>>> {
...
async function recheck(
  ...
  return captureAudit(() => service.recheckDueAccounts([target], new AbortController().signal));
}
...
describe("QuotaRechecker upstream reset scheduling", () => {
  ...
  test("keeps the fixed interval when the snapshot carries no reset time", async () => {
    ...
    const event = lines.find((line) => line.event === "quota_recheck_still_exhausted");
  ...
  test("applies the reset schedule when a usage refresh first detects exhaustion", async () => {
    ...
    const resetAt = NOW + 5 * HOUR_MS;
    const service = rechecker(new FakeManager([healthy]), exhaustedSnapshot(resetAt));
