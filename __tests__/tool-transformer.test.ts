...
} from '../src/kiro/transform/tool-transformer.js'
...
describe('convertToolsToCodeWhisperer', () => {
  ...
  test('missing schema defaults to empty object', () => {
    expect(convertToolsToCodeWhisperer([{ name: 'noop' }])[0]?.toolSpecification.inputSchema).toEqual({ json: {} })
  })
  test('description longer than 9216 chars is truncated', () => {
    const result = convertToolsToCodeWhisperer([
      { function: { name: 't', description: 'x'.repeat(10000), parameters: {} } }
    ])
  ...
  test('empty tools array -> empty array', () => expect(convertToolsToCodeWhisperer([])).toEqual([]))
})
...
describe('deduplicateToolResults', () => {
  ...
  test('no duplicates -> unchanged order and content', () => {
    ...
    expect(deduplicateToolResults(results)).toEqual(results)
  })
  test('empty input -> empty output', () => expect(deduplicateToolResults([])).toEqual([]))
})
