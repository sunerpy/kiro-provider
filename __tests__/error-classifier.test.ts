...
describe("classifyError HTTP decisions", () => {
  ...
  test("switches quota responses when possible and otherwise fails without retrying", () => {
    expect(classifyError(error({ status: 402 }), context({ accountCount: 2 }))).toEqual({
      action: "switch",
      status: 402,
    });
  ...
  test("backs off 500 responses four times then switches on the fifth", () => {
    ...
    expect(classifyError(error({ status: 500 }), context({ serverErrorCount: 4 }))).toEqual({
      action: "retry",
      status: 500,
      retryAfterMs: 8_000,
    });
  ...
  test("remaps only the two explicit context overflow messages from 400 to 413", () => {
    const first = classifyError(error({ status: 400, message: "input is too long" }), context());
    const second = classifyError(
  ...
  test("fails an invalid model with its upstream status", () => {
    expect(classifyError(error({ status: 403, reason: "INVALID_MODEL_ID" }), context())).toEqual({
      action: "fail",
      status: 403,
      terminalStatus: 403,
    });
  ...
  test("fails an invalid model without a status as a bad request", () => {
    expect(classifyError(error({ reason: "INVALID_MODEL_ID" }), context())).toEqual({
      action: "fail",
      status: 400,
      terminalStatus: 400,
    });
  ...
  test("fails a suspended account when no alternative exists", () => {
    expect(
      classifyError(error({ reason: "TEMPORARILY_SUSPENDED" }), context({ accountCount: 1 })),
  ...
  test("fails an invalid bearer after its one forced refresh on a single account", () => {
    ...
    // When
    const classification = classifyError(invalidBearer, classificationContext);
...
describe("classifyError network decisions", () => {
  test("retries network failures with exponential backoff", () => {
    expect(
      classifyError(error({ message: "fetch failed: ECONNRESET" }), context({ retryCount: 2 })),
    ).toEqual({ action: "retry", retryAfterMs: 2_000 });
  });

  test("fails non-network errors without an HTTP status", () => {
    expect(classifyError(error({ message: "unexpected parser failure" }), context())).toEqual({
      action: "fail",
      terminalStatus: 500,
    });
  });
  ...
  test("fails a network error after the retry cap", () => {
    expect(
      classifyError(error({ message: "socket disconnected" }), context({ retryCount: 3 })),
